...
void i2c_master_init(SSD1306_t * dev, int16_t sda, int16_t scl, int16_t reset)
{
    ...
    ESP_ERROR_CHECK(i2c_master_bus_add_device(bus_handle, &mpu6050_cfg, &dev_mpu6050));
...
void read_who_i_am(uint8_t reg_addr)
{
    uint8_t who_am_i[1];
	esp_err_t res;
	res = i2c_master_transmit(dev_mpu6050, &reg_addr,1, I2C_TICKS_TO_WAIT);
    if (res != ESP_OK) {
        ESP_LOGE(TAG, "Failed to send register address: %d (%s)", res, esp_err_to_name(res));
    }

	    res = i2c_master_receive(dev_mpu6050,who_am_i, 1, I2C_TICKS_TO_WAIT);
    if (res != ESP_OK) {
        ESP_LOGE(TAG, "Failed to read data: %d (%s)", res, esp_err_to_name(res));
    }

    printf("Mpo6050 how am i : %u\n",who_am_i[0]);

}

void mpu6050_init(uint8_t reg_addr)
{

	ESP_LOGI(TAG, "Initializing I2C for MPU6050");
    ...
    res = i2c_master_receive(dev_mpu6050,pwr_mgmt_1, 1, I2C_TICKS_TO_WAIT);
    if (res != ESP_OK) {
        ESP_LOGE(TAG, "Failed to read data: %d (%s)", res, esp_err_to_name(res));
    }
	...
	uint8_t out_buf[2];
    out_buf[0] = MPU6050_PWR_MGMT_1; // Register address
    out_buf[1] = 0; // Modified value



	res = i2c_master_transmit(dev_mpu6050, out_buf, 2, I2C_TICKS_TO_WAIT);
    if (res == ESP_OK) {
        ESP_LOGI(TAG, "Power Mgnt write successfull");
    } else {
        ESP_LOGE(TAG, "Could not write to MPU6050: %d (%s)", res, esp_err_to_name(res));
    }
...
// esp_err_t mpu6050_read_register(uint8_t reg_addr,uint8_t *data,size_t length) {
// 	esp_err_t res = i2c_master_transmit(dev_mpu6050,&reg_addr)
// }
...
void mpu6050_read_data(int16_t *accel_x, int16_t *accel_y, int16_t *accel_z, int16_t *gyro_x, int16_t *gyro_y, int16_t *gyro_z)
{
    uint8_t reg = MPU6050_ACCEL_XOUT_H; // Start register to read from
    uint8_t data[14]; // Buffer to store data

    esp_err_t res = i2c_master_transmit(dev_mpu6050, &reg, 1, I2C_TICKS_TO_WAIT);
    // if (res != ESP_OK) {
    //     ESP_LOGE("NOTAG", "Failed to write to MPU6050: %d (%s)", res, esp_err_to_name(res));
    //     return;
    // }

    res = i2c_master_receive(dev_mpu6050, data, 14, I2C_TICKS_TO_WAIT);
    // if (res != ESP_OK) {
    //     ESP_LOGE("NOTAG", "Failed to read from MPU6050: %d (%s)", res, esp_err_to_name(res));
    //     return;
    // }
